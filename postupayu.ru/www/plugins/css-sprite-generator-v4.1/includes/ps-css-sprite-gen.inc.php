...
class CssSpriteGen {
    ...
    public function __construct(array $params = array()) {
        ...
        $GenParams = array(
            ...
            'horizontal-offset' => 2,
            'background' => '',
            'image-output' => strtoupper(SYSTEM_IMG_TYPE), /* PNG GIF, JPG */
            'image-num-colours' => 'true-colour',
            'image-quality' => 75,
            'width-resize' => 100,
            'height-resize' => 100,
            'ignore-duplicates' => 'ignore', /* ignore */
            'class-prefix' => 'sprite-',
            'selector-prefix' => '',
            'selector-suffix' => '',
            'wrap-columns' => true,
            'use-transparency' => true,
            'add-width-height-to-css' => true
        );
    ...
    private function ProcessParams(array $genParams) {
        ...
        $oValidation = new Validation($genParams, $aFieldsToIgnore, $aOptionalFields, $aRules);
                ...
                for ($i = 0; $i < count($aFilesInfo); $i++) {
                    ...
                    if (!$oCurrentImage = $this->CreateImage($aFilesInfo[$i]['path'], $aFilesInfo[$i]['ext'])) {
                        ...
                        $oCurrentImage->newImage(
                                $aFilesInfo[$i]['original-width'], $aFilesInfo[$i]['original-height'], new ImagickPixel('#ffffff')
                        );
                    }
                    ...
                    if ($bResize) {
                        if ($this->sImageLibrary == 'imagick') {
                            ...
                            imagecopyresampled(
                                    $oSprite, $oCurrentImage, $aFilesInfo[$i]['x'], $aFilesInfo[$i]['y'], 0, 0, $aFilesInfo[$i]['width'], $aFilesInfo[$i]['height'], $aFilesInfo[$i]['original-width'], $aFilesInfo['original-height']
                            );
                        }
                    }
                    ...
                    if ($this->sImageLibrary == 'imagick') {
                        $oSprite->compositeImage(
                                $oCurrentImage, $oCurrentImage->getImageCompose(), $aFilesInfo[$i]['x'], $aFilesInfo[$i]['y']
                        );
                    } else {
                        // if using GD and already resized the image will have been copied as part of the resize
                        if (!$bResize) {
                            imagecopy(
                                    $oSprite, $oCurrentImage, $aFilesInfo[$i]['x'], $aFilesInfo[$i]['y'], 0, 0, $aFilesInfo[$i]['width'], $aFilesInfo[$i]['height']
                            );
                        }
                    }
